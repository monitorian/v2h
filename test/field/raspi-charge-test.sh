...
usage() {
  cat <<'USAGE'
Usage:
  raspi-charge-test.sh --ip <V2H_IP_ADDRESS> [options]

Options:
  --dry-run              Run charge safety precheck only. Default.
  --execute              Send one charge SetC after safety precheck passes.
...
LOG_FILE="${LOG_DIR}/v2h-charge-${ACTION}-${STAMP}.log"
...

{
  ...
  case "$ACTION" in
    dry-run)
      echo "== charge dry-run =="
      "${V2H_CMD[@]}" control charge "${control_args[@]}"
      ;;
    execute)
      echo "== charge execute =="
      "${V2H_CMD[@]}" control charge "${control_args[@]}" --execute
      echo
